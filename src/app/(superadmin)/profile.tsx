...
const SUPERADMIN_AVATAR_PRESETS = [
...
export default function SuperAdminProfileScreen() {
  ...
  const handleLogout = () => {
    showZeeAlert(
      "Sign Out",
      "Are you sure you want to sign out of ZeePrep SuperAdmin Portal?",
      [
    ...
    try {
      ...
      if (!result.canceled && result.assets && result.assets.length > 0) {
        ...
        if (res.success && res.photoUrl) {
          ...
          showZeeAlert("Success", "SuperAdmin profile photo updated successfully!", [{ text: "OK" }], "success");
        }
      }
    } catch (err) {
  ...
  const handleSelectPreset = async (presetUrl: string) => {
    ...
    try {
      ...
      if (res.success) {
        ...
        showZeeAlert("Success", "SuperAdmin avatar updated!", [{ text: "OK" }], "success");
      }
    } catch (err) {
  ...
  const handleRemovePhoto = async () => {
    ...
    try {
      ...
      if (res.success) {
        ...
        showZeeAlert("Profile Photo Removed", "Avatar reset to initials.", [{ text: "OK" }], "info");
      }
    } catch (err) {
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.scrollContent}>
      <View style={styles.profileHeader}>
        ...
        <Text style={styles.userName}>{user?.name || "System SuperAdmin"}</Text>
        <Text style={styles.userEmail}>{user?.email}</Text>

        <View style={styles.roleChip}>
          <ShieldAlert size={14} color="#DC2626" />
          <Text style={styles.roleChipText}>SUPERADMINISTRATOR • FULL PRIVILEGE</Text>
        </View>
        ...
        >
          ...
          <Text style={styles.changePhotoBtnText}>
            {hasCustomPhoto ? "Change SuperAdmin Photo" : "Upload SuperAdmin Photo"}
          </Text>
        </TouchableOpacity>
      </View>
      ...
      <View style={styles.section}>
        <Text style={styles.sectionTitle}>Global Security & IP Telemetry</Text>
        <View style={styles.securityCard}>
          <View style={styles.securityRow}>
            ...
            <View style={{ flex: 1 }}>
              <Text style={styles.securityLabel}>Connected Public IP Address</Text>
              <Text style={styles.securityValue}>
                {user?.lastLoginIp || "Verified Secure Host"}
              </Text>
            </View>
      ...
      <View style={styles.section}>
        <Text style={styles.sectionTitle}>SuperAdmin Operations</Text>
        <TouchableOpacity style={styles.logoutBtn} onPress={handleLogout} activeOpacity={0.8}>
          <LogOut size={20} color={ZEEPREP_THEME.colors.error} />
          <Text style={styles.logoutBtnText}>Sign Out of SuperAdmin Portal</Text>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalCard}>
            <View style={styles.modalHeader}>
              <Text style={styles.modalTitle}>SuperAdmin Profile Photo</Text>
            ...
            <View style={styles.presetsGrid}>
              {SUPERADMIN_AVATAR_PRESETS.map((preset, idx) => (
                <TouchableOpacity
                  key={idx}
